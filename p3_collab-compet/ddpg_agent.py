...
class Agent():
    ...
    def step_and_buff(self, state, action, reward, next_state, done,ts):
        """Save experience in replay memory, and use random sample from buffer to learn."""
        # Save experience / reward
#        for states, actions, rewards, next_states, dones in zip(state, action, reward, next_state, done):
#            self.memory.add(states, actions, rewards, next_states, dones,2 )
#        print(" In STEP NUM_AGENTS \t",NUM_AGENTS)
        self.memory.add(state, action, reward, next_state, done,self.NUM_AGENTS)

        # Learn, if enough samples are available in memory
        if len(self.memory) > BATCH_SIZE and ts%NUM_STEPS_TO_UPDATE ==0:
#        if ts%NUM_STEPS_TO_UPDATE ==0:
#            if len(self.memory) > BATCH_SIZE:
                for _ in range(NUM_STEPS_TO_UPDATE):
...
#        state = torch.from_numpy(state).float().unsqueeze(0).to(device)
        ...
        if add_noise:
            for i in range(self.NUM_AGENTS):
                action_i = action[i]
                for action_j in action_i:
                    action_j += self.noise[i].sample()
